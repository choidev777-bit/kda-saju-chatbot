...
| 만세력 계산 | MVP: 간단 계산/어댑터, 확장: npm 패키지 `@fullstackfamily/manseryeok` 또는 공공데이터 API |
...
- [ ] `@fullstackfamily/manseryeok`를 사용할 경우 GitHub ZIP이 아니라 npm 패키지로 설치한다.
- [ ] Node helper를 사용할 경우 `scripts/calculate_saju.mjs`를 만들고 Python tool에서 `subprocess`로 호출한다.
...
| `@fullstackfamily/manseryeok`와 Python 연동이 어렵다 | 중간 | 중간 | Node helper 호출 구조를 단순화하거나 Python 기반 계산 또는 공공 API로 전환한다. |
